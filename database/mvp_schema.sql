-- AI Mentor MVP database schema.
-- Target database: PostgreSQL 16+.
-- This script defines the first vertical slice only:
-- users, student profiles, skills, lessons, tasks, submissions,
-- personal roadmaps, and AI mentor conversations.
...
CREATE INDEX student_skills_student_id_idx
    ON student_skills (student_id);

CREATE INDEX student_skills_skill_id_idx
    ON student_skills (skill_id);

CREATE INDEX skills_status_idx
    ON skills (status);

CREATE INDEX lessons_status_difficulty_idx
    ON lessons (status, difficulty);

CREATE INDEX lessons_primary_skill_id_idx
    ON lessons (primary_skill_id);

CREATE INDEX tasks_status_difficulty_idx
    ON tasks (status, difficulty);

CREATE INDEX tasks_lesson_id_idx
    ON tasks (lesson_id);

CREATE INDEX tasks_primary_skill_id_idx
    ON tasks (primary_skill_id);

CREATE INDEX submissions_student_submitted_at_idx
...
CREATE INDEX roadmaps_student_id_idx
    ON roadmaps (student_id);

CREATE UNIQUE INDEX roadmaps_one_active_per_student_idx
    ON roadmaps (student_id)
    WHERE status = 'active';

CREATE INDEX roadmap_steps_roadmap_status_position_idx
    ON roadmap_steps (roadmap_id, status, position);

CREATE INDEX roadmap_steps_skill_id_idx
    ON roadmap_steps (skill_id);

CREATE INDEX roadmap_steps_lesson_id_idx
    ON roadmap_steps (lesson_id);

CREATE INDEX roadmap_steps_task_id_idx
    ON roadmap_steps (task_id);

CREATE INDEX mentor_conversations_student_updated_at_idx
    ON mentor_conversations (student_id, updated_at DESC);

CREATE INDEX mentor_conversations_context_idx
    ON mentor_conversations (context_type, context_id);

CREATE INDEX mentor_messages_conversation_created_at_idx
